...
from lk.models import Favorite, UserProfile

from .models import Message, Pair_room
...
def chat_room(request, room_name):
    ...
    is_favorite = Favorite.objects.filter(
        user=request.user.userprofile, favorite_user=interlocutor
    ).exists()
...
def toggle_favorite(request, user_id):
    ...
    favorite, created = Favorite.objects.get_or_create(
        user=request.user.userprofile, favorite_user=target_user
    )
    if not created:
        favorite.delete()
