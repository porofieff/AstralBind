...
def profile_edit(request):
    ...
    if request.method == "POST":
        ...
        user_profile.hide_hobby_description = (
            request.POST.get("hide_hobby_description") == "True"
        )
        ...
        self_education_vs_hobby = float(
            translate[request.POST.get("education_vs_hobby")]
        )
        self_education_vs_zodiac = float(
            translate[request.POST.get("education_vs_zodiac")]
        )
        self_hobby_vs_zodiac = float(translate[request.POST.get("hobby_vs_zodiac")])

        self_matrix = matrix_filling(
            self_city_vs_hobby,
            self_city_vs_zodiac,
            self_city_vs_education,
            self_education_vs_hobby,
            self_education_vs_zodiac,
            self_hobby_vs_zodiac,
        )
...
def profile_view(request):
    ...
    comments = (
        Comment.objects.filter(target_user=user_profile)
...
def start_chat(request, user_id):
    ...
    if not room:
        ...
        room = Pair_room.objects.create(
            room_name=room_name, user1=current_user, user2=target_user
        )
        Message.objects.create(
            room=room, sender=current_user, receiver=target_user, message="Чат начат!"
        )
...
def evaluate_user(request):
    ...
    if (
        not user_profile.sex
        or not request.user.first_name
        or not request.user.last_name
    ):
        return redirect("profile_edit")
    favorite_users = Favorite.objects.filter(user=user_profile).values_list(
        "favorite_user_id", flat=True
    )
    ...
    users = UserProfile.objects.exclude(
        Q(user=request.user) | Q(id__in=favorite_users)
    ...
    return render(
        request,
        "evaluate_user.html",
        {"user_profile": random_user, "is_final": is_final},
    )
    ...
    for i, rating in enumerate(ratings):
        try:
            ...
            is_favorite = Favorite.objects.filter(
                user=user_profile, favorite_user=user
            ).exists()
...
def toggle_favorite(request, user_id):
    try:
        ...
        favorite = Favorite.objects.filter(
            user=user_profile, favorite_user=target_user
        ).first()
        if favorite:
            favorite.delete()
        else:
...
def favorite_list(request):
    ...
    favorites = Favorite.objects.filter(user=user_profile).select_related(
        "favorite_user"
    )
    favorites_data = []
    ...
    for favorite in favorites:
        ...
        is_mutual = Favorite.objects.filter(
            user=target_user, favorite_user=user_profile
        ).exists()
        room = Pair_room.objects.filter(
...
def add_comment(request, user_id):
    ...
    is_mutual = Favorite.objects.filter(
        user=target_user, favorite_user=author_profile
    ).exists()
    ...
    if not is_mutual or message_count <= 10:
        return HttpResponseForbidden(
            "У вас нет прав оставлять комментарии этому пользователю"
        )
    ...
    return render(
        request, "add_comment.html", {"form": form, "target_user": target_user}
    )
...
def user_profile(request, user_id):
    target_user = get_object_or_404(UserProfile, user__id=user_id)
    comments = Comment.objects.filter(target_user=target_user).select_related(
        "author__user"
    )
    return render(
        request, "user_profile.html", {"target_user": target_user, "comments": comments}
    )
